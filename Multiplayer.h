...
///@brief hide the characters while giving input
...
/**
 * @brief This file has the code for multipalyer of hangman game
 * @file Multiplayer.h
 * @see multi()
 */
int multi()
{
	system("cls");
	///@brief VARIABLE DECLERATION.......
	char word[20];
	///@brief Word to be guessed
	char temp[20];
	...
	printf("\n");
	///@brief replacting characters with  '-'
	for (i = 0; i < n; ++i)
	{
		temp[i] = '-';
		printf("\t%c", temp[i]);
	...
	while (lives <= 5)
	{
		...
		for (j = 0; j < n; j++)
		{
			...
			if (player_guessed == word[j])
			///@brief checking if user guessed is correct or not
			{
				temp[j] = word[j];
				correct = 1;
				guess++;
		...
		printf("\n");
		printf("\n");
		///@brief replace the correct alphabet
		for (i = 0; i < n; i++)
		{
			printf("\t%c", temp[i]);
		}
		printf("\n");
		printf("\n");
		///@brief player successfully guessed
		if (guess == n)
		{
			printf(" |\n");
	...
	getch();
	Hangman();
	///@brief calling Hangman from hangman.h
	///@see Hangman()
	return 0;
}
