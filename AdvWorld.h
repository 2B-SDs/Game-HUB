...
/**
 * @brief This file contains code for the adventurous world
 * @file AdvWorld.h
 */
int generaterandomnuber(int n)
{
    srand(time(NULL));
    ///@brief srd takes seed as input and is defined into stdlib.h.
    ///@file stdlib.h
    ///@return random number
    return rand() % n;
}
///@brief for rock paper scissors comparision
...
int aworld()
{
    ...
    if (p1score > compscore || p1score == compscore)
    {
        ...
        Food();
        ///@brief to generate food coordinates initially

        life = 3;
        ///@brief number of extra lives

        bend[0] = head;

        Move();
        ///@brief initialing initial bend coordinate
    }
    else
    {
        Sleep(700);
...
void ExitGame()
{
    int i, check = 0;
    for (i = 4; i < length; i++)
    ///@brief starts with 4 because it needs minimum 4 element to touch its own body
    {
        if (body[0].x == body[i].x && body[0].y == body[i].y)
        {
            check++;
            ///@brief check's value increases as the coordinates of head is equal to any other body coordinate
        }
        if (i == length || check != 0)
            break;
    }
...
void Food()
{
    ...
    /**
     * @brief to create food for the first time coz global variable are initialized with 0*/
    {
        food.x = rand() % 70;
        if (food.x <= 10)
            food.x += 11;
    ...
    for (i = bend_no; i >= 0 && len < length; i--)
    {
        if (bend[i].x == bend[i - 1].x)
        {
            ...
            if (diff < 0)
                ...
                {
                    /**
                     * @brief GotoXY(bend[i].x,(bend[i].y-j));
                    printf("*");*/
                    body[len].x = bend[i].x;
                    body[len].y = bend[i].y - j;
                    GotoXY(body[len].x, body[len].y);
        ...
        else if (bend[i].y == bend[i - 1].y)
        {
            ...
            if (diff < 0)
                ...
                {
                    /**
                     * @brief GotoXY((bend[i].x+j),bend[i].y);
                    printf("*");*/
...
///@brief Array to store the words
bool used[NUM_WORDS];
///@brief Array to track which words have been used
int i, j, numCorrect = 0;
///@brief Loop variables and score counter
char guess[WORD_LENGTH];
///@brief Variable to store player's guess
time_t startTime, endTime;
///@brief Variables to store the star
...
void wordgame()
{
    ...
    ///@brief Populate the array of words
    ...
    ///@brief Display the words to the player
